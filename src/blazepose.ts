...
import type { BodyResult, Box, Point } from './util/result';
import type { GraphModel, Tensor } from './util/types';
import type { Config } from './util/config';
import * as coords from './blazeposecoords';

export * as draw from './util/draw';
export * as util from './util/util';

type Keypoint = { score: number, part: string, position: Point, positionRaw: Point };
...
function calculateBoxes(config: Config, keypoints: Array<Keypoint>, outputSize: [number, number]): { keypointsBox: Box, keypointsBoxRaw: Box } {
...
function rescaleKeypoints(keypoints: Array<Keypoint>, outputSize: [number, number]): Array<Keypoint> {
  for (const kpt of keypoints) {
    ...
    kpt.positionRaw = [
      kpt.position[0] / outputSize[0], kpt.position[1] / outputSize[1], kpt.position[2],
    ];
  }
  return keypoints;
}
...
async function detectParts(input: Tensor, config: Config, outputSize: [number, number]): Promise<BodyResult | null> {
  ...
  const keypointsRelative: Array<Keypoint> = [];
  ...
  for (let i = 0; i < points.length / depth; i++) {
    ...
    const position: Point = [Math.trunc(outputSize[0] * positionRaw[0]), Math.trunc(outputSize[1] * positionRaw[1]), positionRaw[2]];
  ...
  const keypoints: Array<Keypoint> = rescaleKeypoints(keypointsRelative, outputSize); // keypoints were relative to input image which is cropped
  const boxes = calculateBoxes(config, keypoints, [outputSize[0], outputSize[1]]); // now find boxes based on rescaled keypoints
  Object.keys(t).forEach((tensor) => tf.dispose(t[tensor]));
